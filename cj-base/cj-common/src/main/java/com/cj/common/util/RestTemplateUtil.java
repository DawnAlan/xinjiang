...
public class RestTemplateUtil {
     ...
     * @return
     */
    private static RestTemplate getRestTemplate(String requestUrl) {
        if (null == restTemplate) {
            ...
            // 存在响应时候返回\u9ad8\u901f\u5546\u8d38 Unicode 自动转为正常字符串
            //  restTemplate.getMessageConverters().set(1, fastJsonHttpMessageConverter);
        }
        // https
        if (requestUrl.startsWith("https:")) {
            if (!(restTemplate.getRequestFactory() instanceof HttpsClientRequestFactory)) {
    ...
    public static String post(String url, String data) {
        ...
        return getRestTemplate(url).postForObject(url, requestEntity, String.class);
    }

    public static String post1(String url, String data,String token) {
        HttpHeaders headers = new HttpHeaders();
        headers.add("Accept", "*/*");
    ...
    public static String post2(String url, MultiValueMap<String, Object> data,String cookie) {
        HttpHeaders headers = new HttpHeaders();
        headers.add("Accept", "application/json, text/javascript, */*; q=0.01");
        headers.add("Accept-Encoding", "gzip, deflate");
        headers.add("Accept-Language", "zh-CN,zh;q=0.9");
        headers.add("Content-Length", "558");
     ...
     * @return
     */
    public static String getByCookie(String url, String cookie) {
        HttpHeaders headers = new HttpHeaders();
        headers.add("Cookie", cookie);
        headers.add("Accept", " */*");
    ...
    public static String getBySaToken(String url, String token) {
        HttpHeaders headers = new HttpHeaders();
        headers.add("Accept", "application/json");
        headers.add("Content-Encoding", "UTF-8");
        headers.add("Content-Type", "application/json; charset=UTF-8");
    ...
    public static <T> T getWithSession(String url, String sessionId, Class<T> responseType, Object... uriVariables) {
     ...
     * @param paramData
     */
    public static String sendForUrlEncoded(String requestUrl, Object paramData) {
        HttpHeaders headers = new HttpHeaders();
        headers.setContentType(MediaType.APPLICATION_FORM_URLENCODED);
        // 数据对象直接转为map
        Map<String, Object> paramMap = convertClass2Map(paramData, false);
        ...
        ResponseEntity<String> stringResponseEntity = getRestTemplate(requestUrl).postForEntity(requestUrl, httpEntity, String.class);
        return (null != stringResponseEntity && null != stringResponseEntity.getBody()) ? stringResponseEntity.getBody() : "";
    }

    public static String sendForUrlEncoded(String requestUrl, MultiValueMap<String, Object> multiValueMap,String token) {
        HttpHeaders headers = new HttpHeaders();
        headers.add("Accept", "*/*");
        headers.add("Content-Encoding", "gzip, deflate");
        headers.add("Content-Type", "application/x-www-form-urlencoded; charset=UTF-8");
        if(StringUtils.isNotEmpty(token)){
    ...
    /**
     * 将对象反射提取参数名和参数值到map 默认的排序是根据key值进行升序排序
     *
     * @param object        需转换对象
     * @param empty2Default 对应参数值为空的 是否列举 默认保存空值为“”-true 不转换空值-false
     * @return
     */
    private static SortedMap<String, Object> convertClass2Map(Object object, boolean empty2Default) {
            ...
            for (; clazz != Object.class; clazz = clazz.getSuperclass()) {
                ...
                for (Field f : field) {
                    ...
                    if (null == f.get(object)) {
                        if (empty2Default) {
                            paramMap.put(f.getName(), "");
                        }
                    } else {
                        paramMap.put(f.getName(), f.get(object).toString());
                    }
                }
            }
