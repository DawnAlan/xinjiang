...
//	private static int generation1;
    ...
    private double[]daynum={10,10,10};
...
//        double[][] fitness_term = new double[2][6];
        ...
        double[][]waterdemand31=new double[2+nameAgricultureWest.length][calStep];
        double[][]waterdemand41=new double[2+nameAgricultureQushou.length+nameAgricultureEast.length][calStep];




        double [][]waterDemandIndustryQushou1=new double[nameIndustryQushou.length][calStep];
...
//       河东绿化细分
        ...
        copyRowRange(waterDemand1,waterDemand,xnum,calStep);
        copyRowRange(waterdemand31,waterdemand3,xnum,calStep);
        copyRowRange(waterdemand41,waterdemand4,xnum,calStep);
        copyRowRange(waterDemandIndustryQushou1,waterDemandIndustryQushou,xnum,calStep);
        copyRowRange(waterDemandGreenEast1,waterDemandGreenEast,xnum,calStep);
        copyRowRange(waterDemandGreenWest1,waterDemandGreenWest,xnum,calStep);
        copyRowRange(waterDemandGreenQushou1,waterDemandGreenQushou,xnum,calStep);
                ...
                if (m == 0)
                {
                    if (outflow_term[m][t] > minOutflow[m]&&outflow_term[m][t] < minOutflow[m]+ waterdemandQ[0][t]) {
                        ...
                        if (id==2)
                        {
                            ...
                            ecologyWater[m][t]=minOutflow[m]* delatT / 1e4 ;             }
                        //水库1缺水额度
                    ...
                    if (outflow_term[m][t] <= minOutflow[m]) {
                        ...
                        waterSupply[m][t]=0;
                        watersupply_lzz[t]=0;
                        ecologyWater[m][t]=outflow_term[m][t]* delatT / 1e4 ;
                    }
