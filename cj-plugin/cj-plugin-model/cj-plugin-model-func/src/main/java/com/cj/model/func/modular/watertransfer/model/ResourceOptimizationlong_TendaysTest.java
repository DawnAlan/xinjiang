...
//        req.SetReservoirs();
        ...
        int  monthNum=monthday[date[1]-1];
        ...
        {
            period=2;
            daynum=new int[]{10,monthNum-20};
            xnum=1;
        }
        if (date[2]>20)
        {
            period=1;
            daynum=new int[]{monthNum-20};
            xnum=2;
        }
...
//        inflow[1]=new double[]{0,0,0};
        ...
        waterDemand1[0]= new double[]{80,79.99,80};
        waterDemand1[1]= new double[]{71.27,40.45,26.82};
        waterDemand1[2]= new double[]{168.1,95.39,63.26};

        waterDemand1[3]= new double[]{320.82,182.06,120.74};
        waterDemand1[4]= new double[]{320.82,182.06,120.74};

        waterdemand31[0]= new double[]{77.61,44.04,29.21};
        waterdemand31[1]= new double[]{64.20,36.43,24.16};

        waterdemand31[2]= new double[]{89.83,50.98,33.81};
        waterdemand31[3]= new double[]{57.11,32.41,21.49};
        waterdemand31[4]= new double[]{22.46,12.74,8.45};
        waterdemand31[5]= new double[]{9.62,5.46,3.62};

        waterdemand41[0]= new double[]{36.57,20.75,13.76};
        waterdemand41[1]= new double[]{52.67,29.89,19.82};
        waterdemand41[2]= new double[]{126.38,71.72,47.56};
        waterdemand41[3]= new double[]{59.78,33.92,22.5};



        waterdemand41[4]= new double[]{16.26,9.23,6.12};
        waterdemand41[5]= new double[]{14.38,8.16,5.41};
        waterdemand41[6]= new double[]{9.01,5.11,3.39};
        waterdemand41[7]= new double[]{5.77,3.27,2.17};
...
//        }
        ...
        double[][]waterdemand4=new double[9][period];
        ...
        double[] fit=calFit(waterTransferReq.getId(), waterTransferReq.getReservoirs(),period,wl_term,waterDemand,inflow)[0];
...
//                       wl_term = wl_best.clone();
                            ...
                            fitness_term = calFit(waterTransferReq.getId(), waterTransferReq.getReservoirs(),period,wl_term,waterDemand,inflow)[0];
...
//        wl_term = wl_term_best.clone();
        ...
        for (int r=0;r<RNum;r++)
        {
            for (int t = 0; t < period; t++) {
                ...
                if (r==0)
                {
                    inflow_water[r][t]=Double.parseDouble(da1.format(inflow[r][t]*daynum[t]*delatT/1e4));
                    outflow_water[r][t]=Double.parseDouble(da1.format(outflow_term[r][t]*daynum[t]*delatT/1e4));
                    deltawater[r][t]=Double.parseDouble(da1.format((inflow[r][t]-outflow_term[r][t])*daynum[t]*delatT/1e4));
                    ...
                    inflow_water[r][t]=Double.parseDouble(da1.format(inflow[r][t]*daynum[t]*delatT/1e4));
                    inflowwater_toutunhe[t]=Double.parseDouble(da1.format(inflow_toutunhe[t]*daynum[t]*delatT/1e4));
                    outflow_water[r][t]=Double.parseDouble(da1.format(outflow_term[r][t]*daynum[t]*delatT/1e4));

                    deltawater[r][t]=Double.parseDouble(da1.format((inflow_toutunhe[t]-outflow_term[r][t])*daynum[t]*delatT/1e4));
...
//        double[][] fitness_term = new double[2][6];
        ...
        for (int t1=0;t1<demandNum;t1++){
            for (int t2=0;t2<period;t2++){
                waterdemandQ[t1][t2]=waterDemand[t1][t2]*1e4/(delatT*daynum[t2]);
            }
        }
        for (int t = 0; t < period; t++)
        {
            ...
            double termOutflow = deltaV * 1e4 / (delatT*daynum[t]) + inflow[0][t];//水量平衡计算
...
//            double localInflow = inflow[1][t] - inflow[0][t];
            ...
            if (outflow_term[0][t] >=waterdemandQ[0][t]+minOutflow) {
                termOutflow = deltaV * 1e4 / (delatT*daynum[t]) + inflow[1][t] + outflow_term[0][t]-waterdemandQ[0][t];//水量平衡计算
                //上游实际来水
            ...
            if (outflow_term[0][t] >minOutflow&&outflow_term[0][t] <minOutflow+waterdemandQ[0][t]){
                termOutflow = deltaV * 1e4 / (delatT*daynum[t]) + inflow[1][t] +minOutflow;
                //上游实际来水
                inflow_toutunhe[t]= inflow[1][t] + minOutflow;
            }
            if (outflow_term[0][t] >0&&outflow_term[0][t] <=minOutflow){
                termOutflow = deltaV * 1e4 / (delatT*daynum[t]) + inflow[1][t] + outflow_term[0][t];
                //上游实际来水
                inflow_toutunhe[t]=inflow[1][t]+outflow_term[0][t];
            }
            if(outflow_term[0][t] <=0){
                termOutflow = deltaV * 1e4/ (delatT*daynum[t]) + inflow[1][t];
...
//                minOutflow[m] = reservoir[m].outflowMin;
                ...
                if (m == 0)
                {
                    if (outflow_term[m][t] > minOutflow&&outflow_term[m][t] < minOutflow+ waterdemandQ[0][t]) {
                        if (id==1||id==3)
                        {
                            fitness1 += (minOutflow + waterdemandQ[0][t]  - outflow_term[m][t]) * (delatT*daynum[t]) / 1e4;
                        }
                        //水库1缺水额度
                        water_shortage1[m][t] =  Double.parseDouble(da.format((minOutflow + waterdemandQ[m][t]  - outflow_term[m][t]) * (delatT*daynum[t])/ 1e4));
                    ...
                    if (outflow_term[m][t] >= minOutflow + waterdemandQ[0][t])
                    {
                        ...
                        watersupply_lzz[t]=Double.parseDouble(da.format((outflow_term[m][t]- minOutflow)*(delatT*daynum[t]) / 1e4 ));
...
//                levelend[m][t] = wl_term[m][t + 1];
                ...
                else  if(m==1)
                {
                    ...
                    {
                        if (id==1||id==3){
                            fitness1 += (minOutflow + watershortage_allQ[t]  - outflow_term[m][t]) * (delatT*daynum[t])/ 1e4;}
                        //水库2缺水额度
                        water_shortage1[m][t] =  Double.parseDouble(da.format((minOutflow +  watershortage_allQ[t]  - outflow_term[m][t]) * (delatT*daynum[t]) / 1e4));
                        //水库2可供水量
        ...
        for (int t1=0;t1<period;t1++){
            f[t1]=1-water_shortage1[1][t1]/(watershortage_allQ[t1]* (delatT*daynum[t1]));
        }
        //供水量
