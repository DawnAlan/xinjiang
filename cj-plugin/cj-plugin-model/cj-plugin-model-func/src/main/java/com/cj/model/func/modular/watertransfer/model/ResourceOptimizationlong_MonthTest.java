...
public class ResourceOptimizationlong_MonthTest {
    ...
    public ArrayList<WaterTransfer> ResourceOptimizationlong_MonthTest(WaterTransferReq req) throws Exception {
        ...
        if (TimeEnd.compareTo(TimeStart) <= 0) {
            throw new CommonException("请检查时间设置是否合理");
        }
        ...
        double [][]waterDemandIndustryQushou1=new double[nameIndustryQushou.length][12];
        ...
        //3是河西，4是河东
        double[][]waterdemand31=new double[2+nameAgricultureWest.length][12];
...
//      渠首工业的总工业用水
        accumulateRow(waterdemand31[1],waterDemandIndustryQushou1);
//       河西灌溉
...
//       河西绿化细分
        ...
        copyRowRange(waterDemand1,waterDemand,DateStart[1]-1,DateEnd[1]);
        copyRowRange(waterdemand31,waterdemand3,DateStart[1]-1,DateEnd[1]);
        copyRowRange(waterdemand41,waterdemand4,DateStart[1]-1,DateEnd[1]);
        copyRowRange(waterDemandIndustryQushou1,waterDemandIndustryQushou,DateStart[1]-1,DateEnd[1]);
        copyRowRange(waterDemandGreenEast1,waterDemandGreenEast,DateStart[1]-1,DateEnd[1]);
        copyRowRange(waterDemandGreenWest1,waterDemandGreenWest,DateStart[1]-1,DateEnd[1]);
        copyRowRange(waterDemandGreenQushou1,waterDemandGreenQushou,DateStart[1]-1,DateEnd[1]);



        if (inflow[0].length!=waterDemand[0].length||inflow[1].length!=waterDemand[0].length){
...
//        double[][] fitness_term = new double[2][6];
        ...
        if (req.getId()==3&&req.getLevelBeginTth()<req.getLevelEndTth()){
            levelEnd[1] = req.getLevelBeginTth();
        }
...
//        if (fitness_term[0]>0.5){
//            throw new CommonException("请检查水位设置是否合理，请适当降低两库末水位");
//        }

        double[]allwater=new double[period];
        double[][]ecologyWater=new double[2][period];
        ecologyWater[0] = calFit(id, reservoirs, period, wl_term, inflow, waterDemand)[12];
        ecologyWater[1] = calFit(id, reservoirs, period, wl_term, inflow, waterDemand)[13];
...
//                minOutflow[m] = reservoir[m].outflowMin;
                ...
                if (m == 0) {
                    if (outflow_term[m][t] > ecoFlow[m][t] && outflow_term[m][t] < ecoFlow[m][t] + waterdemandQ[0][t]) {
                        ...
                        ecologyWater[m][t]=ecoFlow[m][t]* (delatT * monthday[t]) / 1e4 ;
                    }
                    if (outflow_term[m][t] <= ecoFlow[m][t]) {
                        ...
                        ecologyWater[m][t]=outflow_term[m][t]* (delatT * monthday[t]) / 1e4 ;
                    }
...
//                levelend[m][t] = wl_term[m][t + 1];
        ...
        //供水量
            ...
            for (int t = 0; t < period; t++)
            {
                ...
                if (outflow_term[1][t] < ecoFlow[1][t] +waterdemandQ[1][t]+waterdemandQ[2][t])
                {
                    waterSupply[3][t] = 0;
                    waterSupply[4][t] = 0;
                    double timeLength=(delatT * monthday[t]) / 1e4;
                    Map<String, Object> data=new HashMap<>();
                    switch (order)
                    {
                        case 123:
                            data= setWater( outflow_term[1][t], timeLength,ecoFlow[1][t],waterdemandQ[1][t]);
                            ecologyWater[1][t]= (double)data.get("第一优先");
                            waterSupply[1][t] = (double)data.get("第二优先");
                            waterSupply[2][t] = (double)data.get("第三优先");
                            break;
                        case 132:
                            data= setWater( outflow_term[1][t], timeLength,ecoFlow[1][t],waterdemandQ[2][t]);
                            ecologyWater[1][t]= (double)data.get("第一优先");
                            waterSupply[2][t] = (double)data.get("第二优先");
                            waterSupply[1][t] = (double)data.get("第三优先");
                            break;
                        case 213:
                            data= setWater( outflow_term[1][t], timeLength,waterdemandQ[1][t],ecoFlow[1][t]);
                        ...
                        case 231:
                            ...
                            break;
                        case 312:
                            data= setWater( outflow_term[1][t], timeLength,waterdemandQ[2][t],ecoFlow[1][t]);
    ...
    public void setReservoir(List<CurveParam> data, Reservoir[] reservoir) {
        ...
        this.reservoirs[0].levelFloodLimiting = 1397.41;
        ...
        this.reservoirs[1].levelFloodLimiting = 991.2;
