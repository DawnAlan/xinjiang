...
public class TouTunHe {
     ...
     */
    public Map<String, List<List<PredictInputData>>> getOneStationDataList(ForecastInputParamNew paramNew) throws ParseException, IOException {
        ...
        List<List<PredictInputData>> integration = ((paramNew.getModelType() == 3) ? dataUtils.lzzRainIntegration(paramNew) : new ArrayList<>());//整合雨量站数据转为模型所需类型
        ...
        if (paramNew.getPredictionTime().before(dataDate)) {
            ...
            for (int i = 0; i < QJ.size(); i++) {
                for (int j = 0; j < LZZ.size(); j++) {
                    if (timeUtils.DateCompare(QJ.get(i).getDates(), LZZ.get(j).getDates(), "日")) {
     ...
     */
    public List<Flood> getOneStationFlood(List<List<PredictInputData>> Data, ForecastInputParam param, String stationName) throws IOException, InvalidFormatException {
        param.setVmdK(1);
    ...
    public List<Flood> getTTH(List<Flood> Lzz, List<Flood> qj) {
        ...
        int waterLevel = timeUtils.getSpecificDate(Lzz.get(0).getTime()).get("月") == 7 ? 987 : 988;
        //连续列的赋值
        for (int i = 0; i < Lzz.size(); i++) {
            Flood tth = new Flood();
            ...
            tth.setWaterLevel(tthInList.get(i).getH1());//相应水位
            ...
            tth.setWarningTime((tthInList.get(i).getH1() > waterLevel) ? 1 : 0);//超警时段
            tth.setOutQ(tthInList.get(i).getQOut());//出库流量
