...
//            param = objectToList(param);//读取表格
        ...
        for (int i = 0; i < 10; i++) {
            PredictInputData aa = new PredictInputData();
            aa.setFlow((double) i);
            a.add(aa);
        }
        paramNew.setInflowRunoffs(a);
        ...
        for (int i = 0; i < flowData.size(); i++) {
            flowData.get(i).setFlow(0.0);
        }

        ForecastInputParamNew param = new ForecastInputParamNew();
        param.setPeriodTimeStep(1);
        param.setPeriodTimeNum(24);
        param.setIsSimulation(false);
        param.setPredictionTime(sFormat.parse("2024-07-09 00:00:00"));
        String location = "test";
        List<PredictInputData> input = new ArrayList<>();

        Date date = sFormat.parse("2024-07-09 00:00:00");
        for (int i = 0; i < 10; i++) {
            PredictInputData data = new PredictInputData();
            data.setLocation("test");
            data.setDates(timeUtils.addCalendar(date,"小时",i));
            data.setRainfall((double)i);
            input.add(data);
        }
        for (int i = 15; i < 24; i++) {
            PredictInputData data = new PredictInputData();
        ...
        List<RainFallDto> preRain = new ArrayList<>();
        for (int i = 0; i < 48; i++) {
            RainFallDto rainFallDto = new RainFallDto();
            rainFallDto.setDate(sFormat.format(timeUtils.addCalendar(date,"小时",i)));
            rainFallDto.setArea("面雨量");
            rainFallDto.setRainFall((double)i+100);
            preRain.add(rainFallDto);
        }
        param.setRainFallDtos(preRain);

        System.out.println("done");
        InputUtils.lzzWaterLevel = 100.0;
//        List<PredictInputData> result = dataUtils.getHoursRain(param,input,location);



//        Object[][] a = ExcelTool.readStringExcel("D:\\204\\2.头屯河\\径流预报数据文件\\2023年降雨产流.xlsx","Sheet1");
//        double flowSum = 0;
...
//            Date time = (Date) a[i][0];
//            int hour = timeUtils.getSpecificDate(time).get("小时");
//            int hourBefore = 0;
//            Date time2 = new Date();
//            if(i!=0){
//                time2 = (Date) a[i-1][0];
//                hourBefore = timeUtils.getSpecificDate(time2).get("小时");
//            }
//            if(((hour-hourBefore)<0)||i==a.length-1){
//                if (flowNum==0){
//                    flowNum=1;
//                }
//
...
//                flowNum=0;
//            }else {
//                flowSum=flowSum + (Double)a[i][1];
//                flowNum=flowNum + 1;
//            }
//
//        }
//        Object[][] c = new Object[b.size()][1];
...
//        //楼庄子雨量站List
//        Object[][] lzzRainObject = ExcelTool.readStringExcel("D:\\204\\2.头屯河\\资料\\2.小时尺度数据\\雨量站2022-7.xlsx", "LZZ_RAINFALL_STATION_2023121317");
...
//        //楼庄子水位站List
//        Object[][] lzzFlowObject = ExcelTool.readStringExcel("D:\\204\\2.头屯河\\资料\\2.小时尺度数据\\楼庄子库水位站2022-7.xlsx", "LZZ_GAUGING_STATION_20231213173");
...
//        forecastInputParamNew.setLzzHydrologyParam(lzzParam);
//
//        Object[][] qjObject = ExcelTool.readStringExcel("D:\\204\\2.头屯河\\资料\\2.小时尺度数据\\区间.xlsx", "区间");
...
//        }
//        forecastInputParamNew.setIrrigatedHydrologyParam(qjParam);
//        return forecastInputParamNew;
//    }
