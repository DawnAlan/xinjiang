...
public class WaterResourceAssessment {
    ...
    public Map<String, Object> WaterResourceAssessment(List<AppraiseReq> reqList,List<CurveParam> data) throws Exception {
        ...
        for (int i=0;i<reqList.size();i++){
            if (i==0){
            appraise+="在调度区间内来水预报总水量为"+inflowWater[i]+"万m³,"+"其中生态需水为"+ecologyWater[i]+"万m³,"+"各单位需水总量为"+
                    waterDemand[i]+"万m³,";
            }

             appraise+= schemeName[i]+"在楼庄子的蓄水量为："+dischargeLzz[i]+"万m³,头屯河蓄水量为："+dischargeTth[i]+"万m³,总蓄水量为"+storgeAll[i]+
