...
//            throw new RuntimeException("未能从数据库中获取"+location+"数据");
    ...
    public void irrigateMinuteToHour(Map<String, List<RainFallDto>> inputData) {
        ...
        inputData.put("小渠子雨量站", irrigateListMinuteToHour("小渠子雨量站",xqzStation));
        inputData.put("团结一队雨量站", irrigateListMinuteToHour("团结一队雨量站",tjydStation));
        inputData.put("甘沟雨量站", irrigateListMinuteToHour("甘沟雨量站",ggStation));
        inputData.put("头屯河水库雨量站", irrigateListMinuteToHour("头屯河水库雨量站",tthStation));
    }
     ...
     */
    @SneakyThrows
    public List<RainFallDto> irrigateListMinuteToHour(String station,List<RainFallDto> inputList) {
        double rainFall = 0.0;
        int number = 0;
        List<RainFallDto> resultList = new ArrayList<>();
        if (inputList==null||inputList.isEmpty()){
            RainFallDto info = new RainFallDto();
            info.setDate(tu.DateToString(new Date()));
            info.setArea(station);
        ...
        switch (location) {
            case "3号桥": {
                for (int i = 0; i < l; i++) {
                    ...
                    if (rainFall!=0) rainFall = Math.ceil(rainFall * 2.0) / 2.0;
