...
public class WaterSituationStatisticsTableTodayServiceImpl extends ServiceImpl<WaterSituationStatisticsTableTodayMapper, WaterSituationStatisticsTableToday> implements WaterSituationStatisticsTableTodayService {
    ...
    public RestResponse add(WaterSituationStatisticsTableToday waterSituationStatisticsTableToday) {
        ...
        if(null != dayWaterSituationStatisticsTableLzzList && dayWaterSituationStatisticsTableLzzList.size()>0){
            ...
            TrendsTableParam lzzsc = lzzTableParam.stream().filter(t -> t.getPId().equals(ckllTemp.getId()) && t.getParamName().equals("楼庄子水厂")).collect(Collectors.toList()).get(0);
            ...
            waterSituationStatisticsTableToday.setLzzCkllLzzsc(data_8.stream().filter(t->t.getTableHeadId().equals(lzzsc.getId())).map(DayWaterSituationStatisticsTableLzz::getV).collect(Collectors.toList()).get(0));
        ...
        if(null != dayWaterSituationStatisticsTableTthList && dayWaterSituationStatisticsTableTthList.size()>0){
            ...
            TrendsTableParam sw = tthTableParam.stream().filter(t -> t.getPId().equals("0") && t.getParamName().equals("水位")).collect(Collectors.toList()).get(0);
            TrendsTableParam kr = tthTableParam.stream().filter(t -> t.getPId().equals("0") && t.getParamName().equals("库容")).collect(Collectors.toList()).get(0);
            TrendsTableParam jkTemp = tthTableParam.stream().filter(t -> t.getPId().equals("0") && t.getParamName().equals("进库")).collect(Collectors.toList()).get(0);
            ...
            TrendsTableParam ckTemp = tthTableParam.stream().filter(t -> t.getPId().equals("0") && t.getParamName().equals("出库")).collect(Collectors.toList()).get(0);
            TrendsTableParam ckllTemp = tthTableParam.stream().filter(t -> t.getPId().equals(ckTemp.getId()) && t.getParamName().equals("流量")).collect(Collectors.toList()).get(0);
            TrendsTableParam hd = tthTableParam.stream().filter(t -> t.getPId().equals(ckllTemp.getId()) && t.getParamName().equals("河道流量")).collect(Collectors.toList()).get(0);
            TrendsTableParam bg = tthTableParam.stream().filter(t -> t.getPId().equals(ckllTemp.getId()) && t.getParamName().equals("八钢复线流量")).collect(Collectors.toList()).get(0);
            TrendsTableParam hy = tthTableParam.stream().filter(t -> t.getPId().equals("0") && t.getParamName().equals("红岩供水")).collect(Collectors.toList()).get(0);
            TrendsTableParam ckzdTemp = tthTableParam.stream().filter(t -> t.getPId().equals(ckTemp.getId()) && t.getParamName().equals("浊度")).collect(Collectors.toList()).get(0);
            ...
            waterSituationStatisticsTableToday.setTthCkBg(data_8.stream().filter(t->t.getTableHeadId().equals(bg.getId())).map(DayWaterSituationStatisticsTableTth::getV).collect(Collectors.toList()).get(0));
        ...
        if(null != dayWaterSituationStatisticsTableQsList && dayWaterSituationStatisticsTableQsList.size()>0){
            ...
            TrendsTableParam dgq = qsTableParam.stream().filter(t -> t.getPId().equals(daTemp.getId()) && t.getParamName().equals("东干渠流量")).collect(Collectors.toList()).get(0);
            TrendsTableParam dlq = qsTableParam.stream().filter(t -> t.getPId().equals(daTemp.getId()) && t.getParamName().equals("灯笼渠流量")).collect(Collectors.toList()).get(0);
            ...
            TrendsTableParam ny = qsTableParam.stream().filter(t -> t.getPId().equals("0") && t.getParamName().equals("农业")).collect(Collectors.toList()).get(0);
            TrendsTableParam lh = qsTableParam.stream().filter(t -> t.getPId().equals("0") && t.getParamName().equals("绿化")).collect(Collectors.toList()).get(0);
            TrendsTableParam xh = qsTableParam.stream().filter(t -> t.getPId().equals("0") && t.getParamName().equals("泄洪")).collect(Collectors.toList()).get(0);
            ...
            waterSituationStatisticsTableToday.setQsDlqNy(data_8.stream().filter(t->t.getTableHeadId().equals(ny.getId())).map(DayWaterSituationStatisticsTableQs::getV).collect(Collectors.toList()).get(0));
            waterSituationStatisticsTableToday.setQsDlqLh(data_8.stream().filter(t->t.getTableHeadId().equals(lh.getId())).map(DayWaterSituationStatisticsTableQs::getV).collect(Collectors.toList()).get(0));
            ...
            waterSituationStatisticsTableToday.setQsXh(data_8.stream().filter(t->t.getTableHeadId().equals(xh.getId())).map(DayWaterSituationStatisticsTableQs::getV).collect(Collectors.toList()).get(0));
            Double dlqCount = data_8.stream().filter(t -> t.getTableHeadId().equals(dlq.getId())).map(DayWaterSituationStatisticsTableQs::getV).collect(Collectors.toList()).get(0);
            waterSituationStatisticsTableToday.setQsQh(
                    (waterSituationStatisticsTableToday.getQsZgqHj()==null?0.00:waterSituationStatisticsTableToday.getQsZgqHj())+
                    (dlqCount==null?0.00:dlqCount)+
                    (waterSituationStatisticsTableToday.getQsDlqLh()==null?0.00:waterSituationStatisticsTableToday.getQsDlqLh())+
                    (waterSituationStatisticsTableToday.getQsXh()==null?0.00:waterSituationStatisticsTableToday.getQsXh())
            );
        }
        ...
        if(null != dayWaterSituationStatisticsTableDklList && dayWaterSituationStatisticsTableDklList.size()>0){
            ...
            TrendsTableParam qs = dklTableParam.stream().filter(t -> t.getPId().equals("0") && t.getParamName().equals("渠首管理站")).collect(Collectors.toList()).get(0);
            TrendsTableParam hd = dklTableParam.stream().filter(t -> t.getPId().equals("0") && t.getParamName().equals("河东管理站")).collect(Collectors.toList()).get(0);
            TrendsTableParam hx = dklTableParam.stream().filter(t -> t.getPId().equals("0") && t.getParamName().equals("河西管理站")).collect(Collectors.toList()).get(0);
        ...
        if(null != dayWaterSituationStatisticsTableHdList && dayWaterSituationStatisticsTableHdList.size()>0){
            ...
            TrendsTableParam dgqTemp = hdTableParam.stream().filter(t -> t.getPId().equals("0") && t.getParamName().equals("东干渠")).collect(Collectors.toList()).get(0);
            TrendsTableParam tthnc = hdTableParam.stream().filter(t -> t.getPId().equals(dgqTemp.getId()) && t.getParamName().equals("头屯河农场")).collect(Collectors.toList()).get(0);
            TrendsTableParam tthncXyz = hdTableParam.stream().filter(t -> t.getPId().equals(tthnc.getId()) && t.getParamName().equals("西一支")).collect(Collectors.toList()).get(0);
            TrendsTableParam tthncXrz = hdTableParam.stream().filter(t -> t.getPId().equals(tthnc.getId()) && t.getParamName().equals("西二支")).collect(Collectors.toList()).get(0);
            ...
            TrendsTableParam wync = hdTableParam.stream().filter(t -> t.getPId().equals(dgqTemp.getId()) && t.getParamName().equals("五一农场")).collect(Collectors.toList()).get(0);
            ...
            TrendsTableParam yhdlk = hdTableParam.stream().filter(t -> t.getPId().equals(hyTemp.getId()) && t.getParamName().equals("红岩独立口")).collect(Collectors.toList()).get(0);
            TrendsTableParam spnc = hdTableParam.stream().filter(t -> t.getPId().equals(dgqTemp.getId()) && t.getParamName().equals("三坪农场")).collect(Collectors.toList()).get(0);
            ...
            TrendsTableParam gx = hdTableParam.stream().filter(t -> t.getPId().equals(dgqTemp.getId()) && t.getParamName().equals("新疆格信投资有限公司")).collect(Collectors.toList()).get(0);
            ...
            TrendsTableParam xy = hdTableParam.stream().filter(t -> t.getPId().equals(dgqTemp.getId()) && t.getParamName().equals("西缘产业发展中心")).collect(Collectors.toList()).get(0);
            TrendsTableParam xyD1z = hdTableParam.stream().filter(t -> t.getPId().equals(xy.getId()) && t.getParamName().equals("东一支")).collect(Collectors.toList()).get(0);
            ...
            TrendsTableParam lh = hdTableParam.stream().filter(t -> t.getPId().equals(dgqTemp.getId()) && t.getParamName().equals("绿化")).collect(Collectors.toList()).get(0);
            ...
            waterSituationStatisticsTableToday.setHdTthnkXyz(data_8.stream().filter(t->t.getTableHeadId().equals(tthncXyz.getId())).map(DayWaterSituationStatisticsTableHd::getV).collect(Collectors.toList()).get(0));
            waterSituationStatisticsTableToday.setHdTthncXez(data_8.stream().filter(t->t.getTableHeadId().equals(tthncXrz.getId())).map(DayWaterSituationStatisticsTableHd::getV).collect(Collectors.toList()).get(0));
        ...
        if(null != dayWaterSituationStatisticsTableHxList && dayWaterSituationStatisticsTableHxList.size()>0){
            ...
            TrendsTableParam sgzQzq = hxTableParam.stream().filter(t -> t.getPId().equals(sgzTemp.getId()) && t.getParamName().equals("旗帜渠")).collect(Collectors.toList()).get(0);
            ...
            TrendsTableParam lgqTemp = hxTableParam.stream().filter(t -> t.getPId().equals("0") && t.getParamName().equals("六工渠")).collect(Collectors.toList()).get(0);
            TrendsTableParam lgqYyc = hxTableParam.stream().filter(t -> t.getPId().equals(lgqTemp.getId()) && t.getParamName().equals("园艺场")).collect(Collectors.toList()).get(0);
            ...
            TrendsTableParam lgqLgz = hxTableParam.stream().filter(t -> t.getPId().equals(lgqTemp.getId()) && t.getParamName().equals("六工镇")).collect(Collectors.toList()).get(0);
            ...
            TrendsTableParam jgd = hxTableParam.stream().filter(t -> t.getPId().equals("0") && t.getParamName().equals("净水池")).collect(Collectors.toList()).get(0);
            ...
            waterSituationStatisticsTableToday.setHxSgzQzq(data_8.stream().filter(t->t.getTableHeadId().equals(sgzQzq.getId())).map(DayWaterSituationStatisticsTableHx::getV).collect(Collectors.toList()).get(0));
