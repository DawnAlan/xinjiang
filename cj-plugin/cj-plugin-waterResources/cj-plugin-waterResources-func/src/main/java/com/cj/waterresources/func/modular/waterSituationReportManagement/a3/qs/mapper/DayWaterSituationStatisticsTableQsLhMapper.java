...
public interface DayWaterSituationStatisticsTableQsLhMapper extends BaseMapper<DayWaterSituationStatisticsTableQsLh> {
    ...
    @Select("select * from DAY_WATER_SITUATION_STATISTICS_TABLE_QS_LH WHERE TABLE_HEAD_ID = #{tableHeadId} and TIME != '昨日均' and  TO_CHAR(RECORD_TIME,'YYYY-MM-DD') between = #{startTime} and #{endTime}")
    ...
    List<DayWaterSituationStatisticsTableQsLh> selectListHave(@Param("recordTime")String recordTime, @Param("time")String time);
}
