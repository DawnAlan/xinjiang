...
public interface IrrigatedPlatformDataInfoMapper extends BaseMapper<IrrigatedPlatformDataInfo> {
    ...
    @Select("SELECT * FROM IRRIGATED_PLATFORM_DATA_INFO WHERE MONITOR_ID = #{id} AND TO_CHAR(MONITOR_TIME,'yyyy-MM-dd hh24:MI') BETWEEN #{startTime} AND #{endTime} order by MONITOR_TIME DESC")
    ...
    @Select("SELECT * FROM IRRIGATED_PLATFORM_DATA_INFO WHERE TO_CHAR(MONITOR_TIME,'yyyy-MM-dd hh24:MI') = #{time} and MONITOR_NAME like '%雨量站'")
